...
export async function saveUsuario(input:Partial<Usuario>&{senha?:string}){
  ...
  let row:any;
  if(me.perfil!=='administrador'&&selfUpdate){row={id:me.id}; if(input.senha) row.senha_hash=await sha256(input.senha); if(input.trocar_senha!==undefined) row.trocar_senha=input.trocar_senha;}
  else{row={...input,usuario:input.usuario?.toLowerCase().trim(),ativo:input.ativo??true,atualizado_em:new Date().toISOString()}; if(input.senha) row.senha_hash=await sha256(input.senha); if(!row.id) row.id=uid();}
  delete row.senha;
  if(supabaseConfigured&&navigator.onLine){const {error}=await supabase.from('usuarios').upsert(row); if(error) throw error;}
...
export async function saveSetor(input:Partial<Setor>){ensureAdmin(); const row={id:input.id||uid(),nome:input.nome,tipo:input.tipo||'',ativo:input.ativo??true,atualizado_em:new Date().toISOString()}; if(supabaseConfigured&&navigator.onLine){const {error}=await supabase.from('setores').upsert(row); if(error) throw error;} await put('setores_cache',row); await audit('setores',row.id,input.id?'editar':'cadastrar',row); return row}
...
export async function saveMaquina(input:Partial<Maquina>){ensureAdmin(); const row={id:input.id||uid(),setor_id:input.setor_id,nome:input.nome,codigo:input.codigo||'',ordem:input.ordem||0,ativo:input.ativo??true,atualizado_em:new Date().toISOString()}; if(supabaseConfigured&&navigator.onLine){const {error}=await supabase.from('maquinas').upsert(row); if(error) throw error;} await put('maquinas_cache',row); await audit('maquinas',row.id,input.id?'editar':'cadastrar',row); return row}
...
export async function saveTurno(input:Partial<Turno>){ensureAdmin(); const row={id:input.id||uid(),nome:input.nome,inicio:input.inicio||'00:00',fim:input.fim||'00:00',ativo:input.ativo??true,atualizado_em:new Date().toISOString()}; if(supabaseConfigured&&navigator.onLine){const {error}=await supabase.from('turnos').upsert(row); if(error) throw error;} await put('turnos_cache',row); await audit('turnos',row.id,input.id?'editar':'cadastrar',row); return row}
