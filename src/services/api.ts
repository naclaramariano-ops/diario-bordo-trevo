...
export async function saveTurno(input:Partial<Turno>){ensureAdmin(); const row={id:input.id||uid(),nome:input.nome,inicio:input.inicio||'00:00',fim:input.fim||'00:00',ativo:input.ativo??true,atualizado_em:new Date().toISOString()}; if(supabaseConfigured&&navigator.onLine){const {error}=await supabase.from('turnos').upsert(row,{onConflict:'id'}); if(error) throw error;} await put('turnos_cache',row); await audit('turnos',row.id,input.id?'editar':'cadastrar',row); return row}
